...
async function onSubmit() {
  ...
  loading.value = true
  try {
    const r = await $fetch<{ code: number; msg: string; data?: { token: string } }>('/api/admin/auth/login', {
      method: 'POST',
      body: form,
    })
    if (r.code !== 1 || !r.data) {
...
<template>
  <div class="login-bg">
    <el-card class="login-card">
      <div class="login-title">长安知识付费系统</div>
      <div class="login-sub">总后台管理</div>
      <el-form label-position="top" @submit.prevent="onSubmit">
        <el-form-item label="账号">
          <el-input v-model="form.username" placeholder="请输入管理员账号" size="large" clearable>
